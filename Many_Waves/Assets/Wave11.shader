    ...
    Properties
    {
        ...
        SubShader
        {
            ...
            void vert(inout appdata_full v)
            {
                ...
                float4 res = sin((v.texcoord.x)*4 + _Time.y*3);
                res += sin((v.texcoord.y)*4 + _Time.y*3);
                float4 a = (-pow((2 * res - 1), 2) + 1) /2;

                float4 res2 = sin((v.texcoord.x) * 12 + _Time.y *3);
                res2 += sin((v.texcoord.y) * 12 + _Time.y * 3);
                float4 b = (sin(res2))*2;
