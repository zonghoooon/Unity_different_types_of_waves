Shader "Custom/Wave9"
{
    Properties
    {
        ...
        SubShader
        {
            ...
            {
                float pi = 3.1415926;
                //float4 res = ((v.texcoord.x-(1/4))*2*pi + _Time.y*3);
                //v.vertex.y += (sin(res)+pow(cos(res),2))/2;
                //float4 res = ((v.texcoord.x-(1/4))*2*pi + _Time.y*3);
                //v.vertex.y += (pow(sin(res),1)*3+pow(cos(res),3))/2;
                //float4 res = ((v.texcoord.x-(1/4))*2*pi + _Time.y*3);
                //res += ((v.texcoord.y-(1/4))*2*pi + _Time.y*3);
                //v.vertex.y += exp(sin(res))/2;
                float4 res = ((v.texcoord.x - (1 / 4)) * 2 * pi + _Time.y*4);
                v.vertex.y += asin(sin(res))*3;
            }
