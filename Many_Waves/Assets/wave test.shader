Shader "Custom/Wavet"
{
    Properties
    {
        ...
        SubShader
        {
            ...
            {
                float4 res = sin((v.texcoord.x + v.texcoord.y) * UNITY_PI * 6 + _Time.y * 3);
                v.vertex.y += res * 4 / 3;
                float4 res1 = sin((v.texcoord.x - v.texcoord.y / 2) * UNITY_PI * 6 + _Time.y * 2);
                v.vertex.y += res1 * 2 / 3;
                float4 res2 = sin((-v.texcoord.x + v.texcoord.y * 1.5f) * UNITY_PI * 6 + _Time.y * 3);
                v.vertex.y += res2 * 1 / 3;
